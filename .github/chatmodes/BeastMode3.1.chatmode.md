...
11. Always prefer to use `yarn` to run NPM scripts if a `yarn.lock` file is present in the project root. Only use `npm` if there is no `yarn.lock` file.
...
export class AuthService {
   ...
   * ```
   */
  async canViewPolicies(
    request: IGetInsurancePoliciesV2Request
  ): Promise<boolean> {
    // Implementation
  }
}
...
Always prefer to order properties alphabetically within each section of a class or interface. Here is an invalid sample:

```
/**
 * Policy status lifecycle values.
 */
export enum PolicyStatus {
  IN_FORCE = 'In Force',
  ACTIVE = 'Active',
  CONVERTED = 'Converted',
  IN_FORCE_PREMIUM_PAYING = 'In Force-Premium Paying',
  SCHEDULED = 'Scheduled',
  CANCELLED = 'Cancelled',
  LAPSED = 'Lapsed',
  PEND_CONVERSION = 'Pend Conversion'
}
```

Correct sorting pattern:
